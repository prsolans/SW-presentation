...
jQuery(document).ready(function ($) {
    ...
    $('.mergeology-carousel').waypoint({
        handler: function (direction) {
            var tl = new TimelineMax();
            if (direction == 'down') {
                beginMergeologyCarousel();
            }
        },
        offset: '120%'
    });

    $(".thumbnails-list .thumbnail-container").click(function () {
    ...
    $(".leadership-list .thumbnail-container .leader-info img.plus").each(function () {
        $(this).click(function () {
            var elem = $(this).parent().find("div.hide-for-mobile");
            var nextElem = $(this).closest(".thumbnail-container").next();

            if ($(elem).is(":hidden")) {
                elem.show();
                $(this).addClass("active");

                var height = elem.height() / 1.2;
                $(nextElem).css({ marginTop: height + "px" });
            }
            else {
                elem.hide();
                $(this).removeClass("active");
                $(nextElem).css({ marginTop: "0" });
            }
        });
    });
...
function resetLeadershipList() {
    var $elems = $(".leadership-list .thumbnail-container");
    TweenLite.set($elems, { clearProps: "margin-top" });

    $elems = $(".leadership-list .thumbnail-container .hide-for-mobile");
    TweenLite.set($elems, { clearProps: "all" });
}